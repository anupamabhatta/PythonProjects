...
def board_passengers(starship, passengers):
    ...
    passengers_to_board = passengers[:max_passengers]
    starship["passengers_on_board"] = passengers_to_board
    return starship
...
def convert_gravity_value(value):
    ...
    try:
        if value is not None:
            cleaned_value = value.lower().replace("standard", "").strip()
            return utl.convert_to_float(cleaned_value)
...
def create_droid(data):
    ...
    return {
        ...
        "equipment": utl.convert_to_list(droid.get("equipment"), delimiter="|"),
...
def create_person(data, planets=None):
    ...
    if homeworld_url:
        mandalorian_planet = get_mandalorian_data(planets, homeworld_url)
        if mandalorian_planet:
            person["homeworld"] = create_planet(mandalorian_planet)
        else:
            try:
                swapi_planet = get_swapi_resource(homeworld_url)
                person["homeworld"] = create_planet(swapi_planet)
            except Exception as e:
...
def create_planet(data):
    ...
    return {
        ...
        "climate": utl.convert_to_list(planet.get("climate"), delimiter=", "),
        "terrain": utl.convert_to_list(planet.get("terrain"), delimiter=", "),
...
def create_starship(data):
    ...
    return {
        ...
        "armament": utl.convert_to_list(starship.get("armament"), delimiter="|"),
...
def create_vehicle(data):
    ...
    return {
        ...
        "armament": utl.convert_to_list(vehicle.get("armament"), delimiter="|"),
...
def update_planets_visited(data, planet):
    ...
    if planet not in planets_visited:
        planets_visited.append(planet)
    data["planets_visited"] = planets_visited
    return data
...
def main():
    ...
    swapi_tatooine = get_swapi_resource(SWAPI_PLANETS, params={"search": "Tatooine"})["results"][0]
    ...
    razor_crest = create_starship(mandalorian_starships[0])

    # Problem 5.3  Write to file
    utl.write_json("stu-razor_crest.json", razor_crest)

    # Problem 5.5-5.6 Test < create_vehicle >
    swapi_sand_crawler = get_swapi_resource(SWAPI_VEHICLES, params={"search": "Sand Crawler"})[
        "results"
    ][0]
    ...
    cara_dune = create_person(get_mandalorian_data(mandalorian_people, "Carasynthia Dune"), mandalorian_planets)
