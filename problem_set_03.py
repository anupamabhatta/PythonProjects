...
for i in range(len(new_tv_shows)):
    if "netflix" in new_tv_shows[i][5].lower():
...
# TODO 2.2
for show_info in tv_shows:
    show_info_list = show_info.split(" | ")
    cleaned_tv_shows.append(show_info_list)
...
print(f"\n2.5: cleaned_tv_shows (first 5 elements) = {cleaned_tv_shows[:5]}")
...
# TODO 3.2
for show_info in cleaned_tv_shows:
    if show_info[1].strip().lower() == "crime":
        crime_genre.append([show_info[0], show_info[2], show_info[4]])
...
# TODO 5.2
for show_info in cleaned_tv_shows:
    if show_info[4] > avg_rotten_tomato_ratings:
        above_avg_rotten_tomato_ratings.append(show_info[0])
...
# TODO 6.2
for i in range(len(cleaned_tv_shows)):
    if cleaned_tv_shows[i][-1].lower() == "netflix":
...
# TODO 7.2
for show_info in cleaned_tv_shows:
    year = int(show_info[2])
    if year > max_year:
        max_year = year
        newest_show = show_info[0]
    if year < min_year:
        min_year = year
        oldest_show = show_info[0]
...
# TODO 8.2
for i in range(len(cleaned_tv_shows)):
    genre = cleaned_tv_shows[i][1].lower().strip()
