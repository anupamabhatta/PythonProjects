...
% Student Name: Anupama Bhatta

% Student Unique Name: axbhatta

% Lab Section 00X: 003

% I worked with the following classmates: I worked alone
...
def make_move(player, board):
    ...
    while True:
        try:
            move = int(
                input(f"{player_name(player)}'s move: "))
            if 1 <= move <= 9:
                index = move - 1  # Convert move to board index (0-based)
                if board[index] == 0:  # Check if space is unoccupied
...
def check_win_horizontal(board):
    ...
    """
    Check for a win condition in the horizontal direction of a tic-tac-toe board.

    Parameters
    ----------
    board: list, a 9-element list representing the tic-tac-toe board.

    Returns
    -------
    int: The player number (1 or 2) who has won horizontally, or 0 if there is no winner.
    """
    if (board[0] != 0 and
        board[0] == board[1] and
            board[0] == board[2]):
...
def check_win_vertical(board):
    # TODO: write docstring
    """Checks for a vertical win condition on the board.
    ...
    Returns
    -------
        int: The ID of the winning player (1 or 2) if a vertical win is found,
            otherwise 0.
    """
...
def check_win_diagonal(board):
    # TODO: write docstring
    """Checks for a diagonal win condition on the board.
    ...
    Returns
    -------
        int: The ID of the winning player (1 or 2) if a diagonal win is found,
            otherwise 0.
    """
